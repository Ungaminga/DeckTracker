...
namespace DeckTracker.Domain
{
    ...
    ///     when changes are made to their properties: they are reordered automatically when necessary to keep them sorted.
    /// </summary>
    /// <remarks>
    ...
    ///     no unit tests. There could well be bugs in this code.</para>
    /// </remarks>
    public class ObservableSortedList<T> : IList<T>, INotifyPropertyChanged, INotifyCollectionChanged
        where T : class, INotifyPropertyChanged
    {
        ...
        /// <summary>Constructor.</summary>
        public ObservableSortedList(IComparer<T> comparer = null) : this(4, comparer)
        {
        }
        ...
        public void Add(T item)
        {
            ...
            if (i < 0)
                ...
                do i++; while (i < list.Count && comparer.Compare(list[i], item) == 0);
        ...
        public int IndexOf(T item)
        {
            ...
            // Search downwards
            for (int s = i - 1; s >= 0 && comparer.Compare(list[s], item) == 0; s--)
                if (ReferenceEquals(list[s], item))
                    return s;
            // Search upwards
            for (int s = i + 1; s < list.Count && comparer.Compare(list[s], item) == 0; s++)
                if (ReferenceEquals(list[s], item))
                    return s;
            // Not found
            return -1;
        ...
        private void ItemPropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            if (Count <= 1 || (oldIndex == 0 || comparer.Compare(list[oldIndex - 1], item) <= 0)
                && (oldIndex == Count - 1 || comparer.Compare(item, list[oldIndex + 1]) <= 0))
                return;

            // Find where it should be inserted 
            list.RemoveAt(oldIndex);
            int newIndex = list.BinarySearch(item, comparer);
            if (newIndex < 0)
                newIndex = ~newIndex;
            else
                do newIndex++; while (newIndex < list.Count && comparer.Compare(list[newIndex], item) == 0);
